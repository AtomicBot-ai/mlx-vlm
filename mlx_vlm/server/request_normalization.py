...
def _template_kwargs(request) -> dict:
    """Read the OpenAI/HF-TGI ``chat_template_kwargs`` envelope.

    Most OpenAI-compatible clients ship their thinking toggle nested there
    (``{"chat_template_kwargs": {"enable_thinking": false}}``) rather than as a
    top-level field, so the toggle silently no-ops unless we look inside.
    """
...
def _build_gen_args(
    ...
    if "enable_thinking" in template_kwargs:
        enable_thinking = bool(template_kwargs["enable_thinking"])
        template_reasoning = enable_thinking
    elif _request_field_is_set(request, "enable_thinking"):
