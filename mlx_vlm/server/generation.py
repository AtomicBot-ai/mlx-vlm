...
def _collect_stop_tokens(config: Any, processor: Any, model_path: str) -> set[int]:
    ...
    Sources merged in this order — none mandatory, all union'd:
      ...
      4. ``processor.additional_eos_token_ids``
                                              # processor-declared chat stops
      5. ``<model_path>/generation_config.json::eos_token_id``
    ...
    # Processors may declare extra chat-stop ids of their own (e.g. Idefics3's
    # ``<end_of_utterance>``); they are not present in any config file.
    stop_tokens |= _coerce_eos_to_set(
        list(getattr(processor, "additional_eos_token_ids", ()) or ())
    )
